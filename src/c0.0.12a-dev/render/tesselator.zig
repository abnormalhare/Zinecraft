...
pub const Tesselator = struct {
    ...
    pub fn vertex(self: *Tesselator, x: f32, y: f32, z: f32) void {
        ...
        self.vertices += 1;
        if (@rem(self.vertices, 4) == 0 and self.p >= MAX_FLOATS - self.len * 4) {
            self.flush();
        }
    }

    pub fn colori(self: *Tesselator, c: i32) void {
        const r = @as(f32, @floatFromInt((c >> 16) & 255)) / 255.0;
        const g = @as(f32, @floatFromInt((c >> 8) & 255)) / 255.0;
        const b = @as(f32, @floatFromInt((c >> 0) & 255)) / 255.0;
        self.color(r, g, b);
    }

    pub fn set_no_color(self: *Tesselator) void {
        self.no_color = true;
    }
};
