...
const LevelGen = @import("level_gen.zig").LevelGen;
...
pub const Level = struct {
    ...
    pub fn new(alloc: std.mem.Allocator, rand: *std.Random, level_alloc: std.mem.Allocator, w: i32, h: i32, d: i32) !Level {
        ...
        if (!map_loaded) {
            ...
            var level_gen = LevelGen.new(rand, w, h, d);
            self.blocks = try level_gen.generate_map(alloc);
        }
        ...
        for (x0s..x1s) |x| {
            for (y0s..y1s) |y| {
                for (z0s..z1s) |z| {
                    ...
                    const aabb = tile.?.get_aabb(ix, iy, iz);
