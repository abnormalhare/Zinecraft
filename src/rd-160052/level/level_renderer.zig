...
pub const LevelRenderer = struct {
    ...
    pub fn new(alloc: std.mem.Allocator, io: std.Io, level: *Level) !*LevelRenderer {
    ...
    pub fn pick(self: *LevelRenderer, player: *Player) void {
        ...
        var x: i32 = x0;
        while (x < x1) : (x += 1) {
            gl.glPushName(@intCast(@as(u32, @bitCast(x))));

            var y: i32 = y0;
            while (y < y1) : (y += 1) {
                gl.glPushName(@intCast(@as(u32, @bitCast(y))));

                var z: i32 = z0;
                while (z < z1) : (z += 1) {
                    gl.glPushName(@intCast(@as(u32, @bitCast(z))));
    ...
    pub fn render_hit(self: *LevelRenderer, h: HitResult) void {
        const curr_time = std.Io.Clock.now(.real, self.io);
        const t = &TesselatorFile.instance;
